...
const EditTaskPage = () => {
  const { task } = useLoaderData() as { task: PricemeTaskItem | null };
  const [fileInfo, setFileInfo] = useState<FileInfo | null>(null);
  const uploader = useUploader({ onFinishUpload: setFileInfo, from: 'priceme' });
      ...
      <Form method="post">
        ...
        <Box display="flex" gap={2} flexDirection="column">
          ...
          {fileInfo?.url ? (
            <Box
              component="img"
              src={import.meta.env.VITE_SERVER_URL_PRICEME + fileInfo.url}
              width={250}
              height={250}
        ...
        <TextField margin="normal" fullWidth label="Link" name="link" defaultValue={task.link} />
        <TextField margin="normal" fullWidth label="Solution" name="solution" defaultValue={task.solution} />
        <TextField
          margin="normal"
          fullWidth
          label="Description"
